            	...
            	<form action="" method="post">
					...
					if($valid) {
						foreach ($wpdb->get_results($SQL) as $key => $row) {
					 	...
					 	/*
						if($dealership == "") {
							echo '<span style="color:red;" class="glyphicon glyphicon-exclamation-sign"></span><span style="color:red;"> Dealer Name Required!</span>';
						}
						else {
							echo $dealership; 
						}
...
?>
    ...
    <button type="submit" class="btn btn-primary" style="margin:20px 0px 10px 0px;">Add New Score Type</button>
    </form>	
<?php	
}
else {
...
if($valid) {
		foreach ($wpdb->get_results($SQL3) as $key => $row) {
            ...
            <tr>
            	<form action="" method="post">
                    <input type="hidden" name="listaction" value="edit-scores">
